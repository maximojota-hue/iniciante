...
- Para lotes de personagens 3D, quando o usuario colar `CRIAR_VARIOS_POSTS_PERSONAGEM_3D_COM_PASTAS`, tratar cada item do JSON como um post separado. A pasta mae indica a categoria do conjunto de personagens, por exemplo `pokemon`; cada subpasta indica um personagem/modelo e deve ter fotos e um `.url`. Criar os rascunhos em sequencia, atualizando `CONTROLE_POSTS.md` para cada post.
- O preparador de personagens pode incluir ate 3 afiliados no pacote. Se o pacote trouxer `afiliados`, inserir esses produtos no post com imagem clicavel, abrindo em nova aba e `rel="noopener noreferrer sponsored"`. Se um novo afiliado for cadastrado pelo preparador, ele deve atualizar `CONTROLE_AFILIADOS.md` e regenerar `AFILIADOS_CADASTRADOS.html`.
